...
alias tech_cmd {
  ...
  unset %element.desc | unset %spell.element | unset %real.name 
  ...
  if (($3 = $1) && ($is_charmed($1) = false))  { 
    if (((%tech.type != heal) && (%tech.type != boost) && (%tech.type != finalgetsuga))) { $set_chr_name($1) | query %battlechan 4 $+ %real.name cannot attack $gender2($1) $+ self with $2 $+ ! | unset %real.name
      halt 
    }
  }
...
alias tech.status {
  ...
  if ($readini($char($3), skills, utsusemi.on) = on) { set %chance 0 | set %statusmessage.display 4 $+ %real.name i's shadow absorbs the %tech.status.type status! } 
